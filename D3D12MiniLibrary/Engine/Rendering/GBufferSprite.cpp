...
HRESULT K3D12::GBufferSprite::InitializeResource(unsigned int spriteWidth, unsigned int spriteHeight)
{
	...
	hr = this->_geometryResource[GEOMETRY_TYPE::SHADOW_MAP].Create(props, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, resDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, &clearValue);
	if (FAILED(hr)) {
		return hr;
	}
	this->_geometryResource[GEOMETRY_TYPE::SHADOW_MAP].SetName("SHADOW_MAP_TEXTURE");
...
{
	{
		...
		_geometryResource.at(GEOMETRY_TYPE::SHADOW_MAP).CreateView(rtDesc, _rtvHeap.GetCPUHandle(static_cast<unsigned int>(GEOMETRY_TYPE::SHADOW_MAP)));
...

	{
		...
		_geometryResource.at(GEOMETRY_TYPE::SHADOW_MAP).CreateView(uavDesc, this->GetUAVCPUHandle(GEOMETRY_TYPE::SHADOW_MAP));
	...
	}
	{
		...
		_geometryResource.at(GEOMETRY_TYPE::SHADOW_MAP).CreateView(srv, _geometryShaderHeap.GetCPUHandle(static_cast<unsigned int>(GEOMETRY_TYPE::DIFFUSE_MAP)));
...

	{
		...
		_geometryRenderTargets.push_back(GetRTVCPUHandle(GEOMETRY_TYPE::SHADOW_MAP));
	}
	return S_OK;
}
...
D3D12_GPU_DESCRIPTOR_HANDLE K3D12::GBufferSprite::GetRTVGPUHandle(K3D12::GEOMETRY_TYPE type)
{
	...
	case GEOMETRY_TYPE::DIFFUSE_MAP:
		...
		break;
	case GEOMETRY_TYPE::SHADOW_MAP:
		return this->_rtvHeap.GetGPUHandle(static_cast<unsigned int>(GEOMETRY_TYPE::SHADOW_MAP));
		break;
...
D3D12_GPU_DESCRIPTOR_HANDLE K3D12::GBufferSprite::GetSRVGPUHandle(GEOMETRY_TYPE type)
{
	...
	case GEOMETRY_TYPE::DIFFUSE_MAP:
		...
		break;
	case GEOMETRY_TYPE::SHADOW_MAP:
		return this->_geometryShaderHeap.GetGPUHandle(static_cast<unsigned int>(GEOMETRY_TYPE::SHADOW_MAP));
		break;
	case GEOMETRY_TYPE::RESULT:
...
D3D12_GPU_DESCRIPTOR_HANDLE K3D12::GBufferSprite::GetUAVGPUHandle(GEOMETRY_TYPE type)
{
	...
	case GEOMETRY_TYPE::DIFFUSE_MAP:
		...
		break;
	case GEOMETRY_TYPE::SHADOW_MAP:
		return this->_uavHeap.GetGPUHandle(static_cast<unsigned int>(GEOMETRY_TYPE::SHADOW_MAP));
		break;
	case GEOMETRY_TYPE::RESULT:
...
D3D12_CPU_DESCRIPTOR_HANDLE K3D12::GBufferSprite::GetRTVCPUHandle(GEOMETRY_TYPE type)
{
	...
	case GEOMETRY_TYPE::DIFFUSE_MAP:
		...
		break;
	case GEOMETRY_TYPE::SHADOW_MAP:
		return  this->_rtvHeap.GetCPUHandle(static_cast<unsigned int>(GEOMETRY_TYPE::SHADOW_MAP));
		break;
	case GEOMETRY_TYPE::RESULT:
...
D3D12_CPU_DESCRIPTOR_HANDLE K3D12::GBufferSprite::GetSRVCPUHandle(GEOMETRY_TYPE type)
{
	...
	case GEOMETRY_TYPE::DIFFUSE_MAP:
		...
		break;
	case GEOMETRY_TYPE::SHADOW_MAP:
		return  this->_geometryShaderHeap.GetCPUHandle(static_cast<unsigned int>(GEOMETRY_TYPE::SHADOW_MAP));
		break;
	case GEOMETRY_TYPE::RESULT:
...
D3D12_CPU_DESCRIPTOR_HANDLE K3D12::GBufferSprite::GetUAVCPUHandle(GEOMETRY_TYPE type)
{
	...
	case GEOMETRY_TYPE::DIFFUSE_MAP:
		...
		break;
	case GEOMETRY_TYPE::SHADOW_MAP:
		return  this->_uavHeap.GetCPUHandle(static_cast<unsigned int>(GEOMETRY_TYPE::SHADOW_MAP));
		break;
	case GEOMETRY_TYPE::RESULT:
...
void K3D12::GBufferSprite::SetLightingResult()
{
	...
	_list->GetCommandList().Get()->SetComputeRootDescriptorTable(static_cast<unsigned int>(SHADER_PARAMATER_INDEX_RESULT::SHADOW_MAP), GetSRVGPUHandle(GEOMETRY_TYPE::SHADOW_MAP));
...
void K3D12::GBufferSprite::TransitionGeometryPassStart()
{
	...
	_geometryResource[GEOMETRY_TYPE::SHADOW_MAP].ResourceTransition(_list->GetCommandList().Get(), D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_RENDER_TARGET);
	...
	_list->GetCommandList().Get()->ClearRenderTargetView(GetRTVCPUHandle(GEOMETRY_TYPE::SHADOW_MAP), _geometryResource[GEOMETRY_TYPE::SHADOW_MAP].GetClearValue().Color, 0, nullptr);
...
void K3D12::GBufferSprite::TransitionGeometryPassEnd()
{
	...
	_geometryResource[GEOMETRY_TYPE::SHADOW_MAP].ResourceTransition(_list->GetCommandList().Get(), D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_GENERIC_READ);
...
void K3D12::GBufferSprite::TransitionLightingPassStart(DepthStencil* depthResource)
{
	...
	_geometryResource[GEOMETRY_TYPE::SHADOW_MAP].ResourceTransition(_list->GetCommandList().Get(), D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
