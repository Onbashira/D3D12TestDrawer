...
void CSMain(uint3 groupeID : SV_GroupID, uint3 dispatchThreadID : SV_DispatchThreadID, uint3 groupThreadID : SV_GroupThreadID)
{
    ...
    for (uint i = 0; i < sTileNumLights; ++i)
    {
        ...
        else
        {
            ...
            rimResult = CalcDirectionalLightRim(surfaceData.depth, surfaceData.posView, surfaceData.normalView, lightViewDirection, float3(1.0f,0.5f,0.0f),2.8f);
    ...
    //適当アンビエント
    float4 ambient = surfaceData.albedo * 0.05f;
    ...
    //}
    //ResultTexture[globalIndex] = float4(saturate(rim), surfaceData.albedo.w);

    ResultTexture[globalIndex] = float4(saturate((diffuse) * surfaceData.albedo.rgb + ambient.rgb + specular + rim), surfaceData.albedo.w);
}
